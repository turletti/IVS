...
*--------------------------------------------------------------------------*
modified by Winston Dang  wkd@Hawaii.Edu	10/1/92
...
FILE *F_loss;
BOOLEAN stat_mode=FALSE; /* Boolean: True if statistical mode */
void (*get_image)();
...
int NbPacketsLost(sock, ForceGobEncoding, GobEncoded, PARTIAL, NG_MAX)
     ...
     int NG_MAX;
  {
    ...
    if(PacketLost[0] || (last_refresh > 60)){
      *PARTIAL=FALSE;
    }else{
      *PARTIAL=TRUE;
      if(!nb_lost){
...
void vfc_get_cif_ntsc(vfc_dev, im_data)
    ...
    u_char im_data[][COL_MAX];
  {
    ...
    */
     ...
     */
    ioctl(vfc_dev->vfc_fd, VFCSCTRL, &grab_ioctl);
/*    ioctl(vfc_dev->vfc_fd, VFCSCTRL, MEMPRST);     */
    /* Skip over the non-active samples */
    for (i=0; i< (VFC_OSKIP_NTSC >> 4  ); i++)
	{
	tmpdata = *data_port;
	tmpdata = *data_port;
	tmpdata = *data_port;
...
int vfc_scan_ports(vfc_dev, format)
     VfcDev  *vfc_dev;
     int     *format;
{
  int in_port = VFC_PORT1;
  
  vfc_set_port(vfc_dev, in_port);
  while((vfc_set_format(vfc_dev, VFC_AUTO, format) < 0) &&
	(in_port <= VFC_PORT1) ) {
    in_port++;
    vfc_set_port(vfc_dev, in_port);
  }
...

{
	...
	if((F_loss=fopen(".videoconf.loss", "w")) == NULL){
      ...
      /* mod - installed function pointer to get the image */
      if(CIF)
	  get_image =(format == VFC_NTSC) ? vfc_get_cif_ntsc :
	      vfc_get_cif_pal;
       else
	   get_image =(format == VFC_NTSC) ? vfc_get_qcif_ntsc:
	   vfc_get_qcif_pal;

      (*get_image)(vfc_dev, new_image);
...
encode_h261(group, port, ttl, new_image, image_coeff, threshold, NG_MAX, 
...

  {
    ...
    int diff_image[LIG_MAX][COL_MAX];
    int cc, quant_mb;
    int k;
    int bnul, mbnul;
    ...
    int nblock_cif_max=800;  /* CIF  : if nb(blocks) detected > this value */
    int nblock_qcif_max=200; /* QCIF :    then use INTRA mode              */
    BOOLEAN NEW_PACKET=FALSE; /* If an image is too big, several packets
				 may be necessary. True if it's the case.
			      */    
    ...
    if(quality){
      ffct_intra = ffct_intra_8x8;
    }else{
    ffct_intra = ffct_intra_4x4;
    }     
    /*************\
    * IMAGE LAYER *
    \*************/
    ...
    printf("QUANT: %d, RT: %d\n", QUANT_FIX, RT);
#endif
    put_value(&chaine_codee[i],16,20,&i,&j); /* CDI */
	...
	if(!ForceGobEncoding[NG-1]){
	  ...
	  for(MBA=1; MBA<34; MBA++){
	    ...
	    /* mod - eliminate the diff var. just an extra calculation */
	    sum =  abs((int)new_image[lig_11][col_11] - (int)old[lig_11][col_11]);
	    sum += abs((int)new_image[lig_11][col_12] - (int)old[lig_11][col_12]);
	    sum += abs((int)new_image[lig_12][col_11] - (int)old[lig_12][col_11]);
	    sum += abs((int)new_image[lig_12][col_12] - (int)old[lig_12][col_12]);
	    if(sum>threshold){
	      Nblock ++;
	      mb->CBC |= 32;
	    }else
	    ...
	    sum =  abs((int)new_image[lig_11][col_21] - (int)old[lig_11][col_21]);
	    sum += abs((int)new_image[lig_11][col_22] - (int)old[lig_11][col_22]);
	    sum += abs((int)new_image[lig_12][col_21] - (int)old[lig_12][col_21]);
	    sum += abs((int)new_image[lig_12][col_22] - (int)old[lig_12][col_22]);
	    if(sum>threshold){
	      Nblock ++;
	      mb->CBC |= 16;
	    }else
	    ...
	    sum =  abs((int)new_image[lig_21][col_11] - (int)old[lig_21][col_11]);
	    sum += abs((int)new_image[lig_21][col_12] - (int)old[lig_21][col_12]);
	    sum += abs((int)new_image[lig_22][col_11] - (int)old[lig_22][col_11]);
	    sum += abs((int)new_image[lig_22][col_12] - (int)old[lig_22][col_12]);
	    if(sum>threshold){
	      Nblock ++;
	      mb->CBC |= 8;
	    }else
	    ...
	    sum =  abs((int)new_image[lig_21][col_21] - (int)old[lig_21][col_21]);
	    sum += abs((int)new_image[lig_21][col_22] - (int)old[lig_21][col_22]);
	    sum += abs((int)new_image[lig_22][col_21] - (int)old[lig_22][col_21]);
	    sum += abs((int)new_image[lig_22][col_22] - (int)old[lig_22][col_22]);
	    if(sum>threshold){
	      Nblock ++;
	      mb->CBC |= 4;
	    }else
	...
	/* mod - use shift operation rather than divide. much faster */
	y_gob = ((NG-1) >> 1)*48;
	if(CIF)
	  gob = (GOB *) (image_coeff)[NG-1];
	else
	...
	for(MBA=1; MBA<34; MBA++){
	  ...
	  /* MACROBLOCK LOOP */
	  /* mod - shift operations are faster than divide */
	  if(MBA > 22){ 
	    col = x_gob + ((MBA-23) << 4);
	    lig = y_gob + 32;
	  }else{
	  ...
	  if(PARTIAL && !ForceGobEncoding[NG-1]){
	    ...
	    if((mb->CBC) & 0x20){
	      register int l, c;
	      for(l=0; l<8; l++){
		register int auxl= lig+l;
		register int auxc;
		register int *pt_image = &(diff_image[auxl][col]);
		for(c=0; c<8; c++){
		  auxc=col+c;
		  *pt_image++ = (int)new_image[auxl][auxc] 
		    - (int)old[auxl][auxc];
		}
	    }

	      max=(*ffct)(diff_image, mb->P[0], lig, col, 1, &cmax0);
	      if(max>max_coeff)
		max_coeff=max;
	    }
	    if((mb->CBC) & 0x10){
		...
		register int auxl= lig+l;
		register int auxc;
		register int *pt_image = &(diff_image[auxl][col+8]);
		for(c=8; c<16; c++){
		  auxc=col+c;
		  *pt_image++ = (int)new_image[auxl][auxc] 
		    - (int)old[auxl][auxc];
	      ...
	      max=(*ffct)(diff_image, mb->P[1], lig, col+8, 1, &cmax1);
	      if(max>max_coeff)
		max_coeff=max;
	    }
	    if((mb->CBC) & 0x08){
		...
		for(c=0; c<8; c++){
		  ...
		  *pt_image++ = (int)new_image[auxl][auxc] 
	      ...
	      max=(*ffct)(diff_image, mb->P[2], lig+8, col, 1, &cmax2);
	      if(max>max_coeff)
		max_coeff=max;
	    }
	    if((mb->CBC) & 0x04){
		...
		for(c=8; c<16; c++){
		  ...
		  *pt_image++ = (int)new_image[auxl][auxc] 
	      ...
	      max=(*ffct)(diff_image, mb->P[3], lig+8, col+8, 1, &cmax3);
	      if(max>max_coeff)
		max_coeff=max;
	    }
	  }else{
	    ...
	    mb->MODE = INTRA;
	    mb->CBC = 60;
	    (*ffct_intra)(new_image, mb->P[0], lig, col, &cmax0);
	    (*ffct_intra)(new_image, mb->P[1], lig, col+8, &cmax1);
	    (*ffct_intra)(new_image, mb->P[2], lig+8, col, &cmax2);
	    (*ffct_intra)(new_image, mb->P[3], lig+8, col+8, &cmax3);
	  ...
	  if(mb->CBC){
	     ...
	     */
	    bnul=0;
	    if(quant_mb & 0x01){	/* same as mod 2 */
	      /* odd quantizer */
	      for(n=0; n<4; n++){
	...
	* H.261 CODING OF GOB *
	...
	put_value(&chaine_codee[i],1,16,&i,&j); /* CDGB = 1 */
	...
	if(ThisGobEncoded[NG-1]){
	  ...
	  for(NM=1; NM<=33; NM++){
	    ...
	    if(mb->QUANT != quant){
	      quant=mb->QUANT;
	      if(CBC_IS_CODED)
		put_value(&chaine_codee[i],1,5,&i,&j); /*TYPEM=INTRA+CBC+QUANTM*/
	      else
		put_value(&chaine_codee[i],1,7,&i,&j); /* TYPEM = INTRA+QUANTM */
	      put_value(&chaine_codee[i],quant,5,&i,&j); /* Coding of QUANTM */
	    }
	    else{
	      if(CBC_IS_CODED)
		put_bit(chaine_codee,1,i,j); /* TYPEM = INTRA+CBC */
	      else
		put_value(&chaine_codee[i],1,4,&i,&j); /* TYPEM = INTRA */
	    }
	    if(CBC_IS_CODED){
...
printf("noGOB %d ", NG);
	...
	if(CUTTING || NEW_PACKET || (NG==NG_MAX)){
	  if(ThisGobEncoded[NG-1] || NEW_PACKET || (NG==NG_MAX)){
	    ...
	    if( 7 - j){
	      ...
	      for(b=0; b<max; b++)
		put_value(&chaine_codee[i], 0x0f, 11, &i, &j);
	    }
	    /* Next number packet */
	    if(NG==NG_MAX){
	      if(stat_mode){
		...
		exit(-1);
	      ...
	      fprintf(stderr, "Cannot send %d bytes\n", i);
	    }
#ifdef STAT
	    printf(":: NUM_PACKET:%d, NB_BYTES:%d\n", num_packet, lwrite);
	    if(NG==NG_MAX)
	      printf("Nb(Blocks)= %d\n", Nblock);
#endif /* STAT */
#ifdef DEBUG
...
int chain_forecast(tab_code,n_code,result,chain,i,j)
...

  {
	...
	put_bit_ptr(chain_ptr,bit,ii,jj);
	k ++;
...
int put_value(chain_ptr, value, n_bits, i, j) 
