...
*                                                                          *
*  ADDED Audio output selection -- 24/9/92 Ian Daniel Rank Xerox EuroPARC  *
*                                                                          *
\**************************************************************************/
...
static Widget video_cif, video_qcif,  ButtonPort, ButtonVideo, ButtonAudio, 
...
BOOLEAN START_EAUDIO=FALSE, START_EVIDEO=FALSE, SPEAKER=TRUE;
...
int T_ALARM=10; /* a Declaration packet is sent every T_ALARM seconds */
...
void CallbackDVideo(w, null, reason)
     Widget w;
     char *null;
     XmPushButtonCallbackStruct *reason;
{
...
void CallbackDAudio(w, null, reason)
     Widget w;
     char *null;
     XmPushButtonCallbackStruct *reason;
{
  ...
  SendPP_NEW_TABLE(t, da_fd);
#elif DECSTATION
  DisplayMsgError("Sorry, DEC audio doesn't work yet ...");
#endif /* SPARCSTATION */
     ...
     LOCAL_TABLE t;
{
  ...
  if((type=GetType(buf)) != CONTROL_TYPE){
    ...
    case P_DECLARATION:
      ...
      strcat(name, "@");
      if((host=gethostbyaddr((char *)(&(from.sin_addr)), fromlen, 
     ...
     char **argv;
{
  ...
  /* Sound to speaker to begin with */
#endif
  if(STAT_MODE){
