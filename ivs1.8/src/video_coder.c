...
/* XXX The following three #defines aren't used!? */
#define NO_SOUND 0
...
encode_h261(group, port, ttl, new_image, image_coeff, threshold, NG_MAX, 
...

  {
    ...
    int max_coeff;
    static int cpt=0;
    int T_im=1000/frequency; /* here not used */
    static int INC_QUANT=0;
    static int num_packet=0;
    static BOOLEAN PARTIAL=FALSE; /* false if all the image has to be encoded*/
    ...
    * IMAGE LAYER *
    ...
    /*------------------------------------------------------------*/
    /* First Pass: Movement detection for each block of the image */
    /*------------------------------------------------------------*/
    ...
    /* If a change of scene happened, choose INTRA mode.
       A change of scene is detected as more than half
       blocks detected.
    */
    if(CIF){
    ...
    /*-----------------------------------------------------*/
    /* Second Pass : H.261 encoding of all detected blocks */
    /*-----------------------------------------------------*/
	...
	for(MBA=1; MBA<34; MBA++){
	  ...
	  if(PARTIAL && !ForceGobEncoding[NG-1]){
	    ...
	    /* Some blocks are encoded in INTER Mode */
	    max_coeff=0;
	    if((mb->CBC) & 0x20){
	      register int l, c;
	      for(l=0; l<8; l++){
	    ...
	    /* All the blocks are coded in INTRA Mode */
	    mb->MODE = INTRA;
	    mb->CBC = 60; /* Warning, in INTRA Mode CBC=63 implicitly */
	...
	if(ThisGobEncoded[NG-1]){
	  ...
	  for(NM=1; NM<=33; NM++){
	    ...
	    if(mb->QUANT != quant){
	      quant=mb->QUANT;
	      if(CBC_IS_CODED)
		put_value(chaine_codee,1,5,&i,&j); /* TYPEM=INTER+QUANTM */
	      else
		put_value(chaine_codee,1,7,&i,&j); /* TYPEM = INTRA+QUANTM */
	      ...
	      if(CBC_IS_CODED)
		put_bit(chaine_codee,1,i,j); /* TYPEM = INTER */
	      else
