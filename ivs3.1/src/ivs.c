...
static BOOLEAN RECV_ONLY=FALSE; /* True if only reception is allowed */
...
static void LoopDeclare()
{
  ...
  SendDESCRIPTOR(csock_s, &addr_s, len_addr_s, START_EVIDEO, START_EAUDIO, 
		 my_feedback_port, my_sin_addr, name_user, class_name);
  alarm((u_int)T_ALARM);
     ...
     XtPointer call_data, client_data;
{
  ...
  do{
	...
	fprintf(stderr, 
		"GetControlInfo: Bad content RTCP packet %d ", buf[l+2]);
        ERROR = TRUE;
        break;
      }
      if(buf[l+4] != RTP_IVS_VERSION){
	fprintf(stderr,
		"GetControlInfo: Bad IVS option %d.%d received ",
		buf[l+4]>>4, buf[l+4]&0x0f);
	ERROR = TRUE;
	break;
      }
      AUDIO_ENCODING = buf[l+5];
      VIDEO_ENCODING = buf[l+6];
      CONTINUE = (buf[l+8] & 0x80) ? FALSE : TRUE;
      if((buf[l+8] & 0x7f) != RTP_SDES){
	fprintf(stderr, "GetControlInfo: SDES missing after FMT ");
        ERROR = TRUE;
        break;
      }
      class = (buf[l+12] << 8) + buf[l+13];
      if(class != 1){
	fprintf(stderr, "GetControlInfo: Bad class address in SDES %d ",
		class);
        ERROR = TRUE;
        break;
      }
      bcopy((char *)&buf[l+16], (char *)&ip_long, 4);
      class = (buf[l+20] << 8) + buf[l+21];
      strcpy(name, (char *) &(buf[l+23]));
    ...
    case RTP_RNA:
      qos_it = buf[l+3];
      if(buf[l+4] || (buf[l+5] != 1)){
	fprintf(stderr, 
		"GetControlInfo: Bad address class RTP_RNA 0x%d%d ", 
		buf[l+4], buf[l+5]);
	ERROR = TRUE;
	break;
      }
      bcopy((char *)&buf[l+8], (char *)&ip_long, 4);
      l += (buf[l+1] << 2);
      break;
    ...
    case RTP_RTA:
      if(buf[l+3] != 17){
	fprintf(stderr, 
		"GetControlInfo: Bad protocol RTP_RNA 0x%d%d ", 
		buf[l+4], buf[l+5]);
	ERROR = TRUE;
	break;
      }
      feedback_port = (u_short)((buf[l+6] << 8) + buf[l+7]);
      l += (buf[l+1] << 2);
      break;
     ...
     char **argv;
{
  ...
  */
  ...
  if(!RECV_ONLY){
	...
	XtAddCallback(menu_video[n], XtNcallback, CallbackMenuVideoFormat, 
		      NULL);
      }else{
#ifndef VIDEOTX
	menu_video[n] = XtCreateManagedWidget(VideoFormatMenuItems[n], 
    ...
    for(n=0; n<5; n++){
      ...
      XtAddCallback(menu_rate[n], XtNcallback, CallbackMenuRateControl, 
		    NULL);
      if(n==2)
	entry = XtCreateManagedWidget("Line", smeLineObjectClass, 
				      menu_control, NULL, 0);
