...
BOOLEAN FIR_USELESS=FALSE; /* True if Full Intra Request not required
			      to start recording */
BOOLEAN FIRST_P;
...
static void Usage(name)
     char *name;
{
  fprintf(stderr, "Usage: %s [-I @IP] [{-m|-u} hostname] [-nofir] filename\n",
	  name);
  exit(1);
}
  ...
  if(RECORDING_REQUEST){
	...
	if(!RECORDING){
	  ...
	  if(FIR_USELESS || (buf[9] & 0x40) != 0){
	    ...
	    SendFIR(hostname, feedback_port);
	  }
	}else{
	  delay = NewDelay(FALSE);
	  save(F_out, buf, lr, delay);
