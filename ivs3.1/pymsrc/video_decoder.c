...
/*printf("decoded MODE_4, after i:%d, j:%d, ii, jj, cpt=%d\n", cpt);*/
      break;
	...
	temp = (temp << 1) | get_bit(chain, ii, jj);
      ...
      *(pt_out+offset) = *(pt_out+offset1) = temp;
      cpt += 4;
      pt_out = block_out + tab_offset[cpt];
/*printf("decoded MODE_3 %d, after i:%d, j:%d, cpt=%d\n", temp, ii, jj, cpt);*/
      break;
	...
	toffset += offset;
      ...
      cpt += 16;
      pt_out = block_out + tab_offset[cpt];
/*printf("decoded MODE_2 %d, after i:%d, j:%d, cpt=%d\n", temp, ii, jj, cpt);*/
      break;
...
int ReadBuffer(sock, buf, i, j)
...

  {
    ...
    static pckt_tot=1; /* STAT: received packets */
...
*/
	...
	video_coding = RTP_H261_CONTENT;
	break;
      case RTP_PYRA_CONTENT:
	video_coding = RTP_PYRA_CONTENT;
	break;
      default:
	...
	if(packet_expected == sequence_number){
...
#endif
	  if(STAT_MODE){
	    aux = (65536 + sequence_number - packet_expected) % 65536;
	    pckt_lost += aux;
	    pckt_tot += aux;
	    fprintf(F_dloss, 
	  ...
	  if(E == FALSE){
		...
		*/
	    ...
	    /* Folowing code to decode must be a GBSC. So we only need to
            *  decode the next packet with Sbit == 1.
	    */
	    packet_expected = (sequence_number + 1) % 65536;
	    if(buf[8] & 0x80){
	  ...
	  * GROUP OF BLOCKS LAYER *
	  ...
	  do{ /* MB LOOP */
	    ...
	    * BLOCK LAYER *
		...
		/* Pyramidal decoding */
		if(P & 0x20){
		  if(decode_block(chaine_codee, &(tab_Y[y_mb][x_mb]), 
				  L_Y, &i, &j) < 0){
		    goto after_gbsc;
		  }
		}
		if(P & 0x10){
		  if(decode_block(chaine_codee, &(tab_Y[y_mb][x_mb+8]), 
				  L_Y, &i, &j) < 0){
		    goto after_gbsc;
		  }
		}
		if(P & 0x08){
		  if(decode_block(chaine_codee, &(tab_Y[y_mb+8][x_mb]),
				     L_Y, &i, &j) < 0){
		    goto after_gbsc;
		  }
		}
		if(P & 0x04){
		  if(decode_block(chaine_codee, &(tab_Y[y_mb+8][x_mb+8]),
				     L_Y, &i, &j) < 0){
		    goto after_gbsc;
		  }
		}
		if(COLOR_ENCODED){
		  if(P & 0x02){
		    if(decode_block(chaine_codee, &(tab_Cb[y_mb2][x_mb2]),
				    L_Y_DIV2, &i, &j) < 0){
		      goto after_gbsc;
		    }
		  }
		  if(P & 0x01){
		    if(decode_block(chaine_codee, &(tab_Cr[y_mb2][x_mb2]),
				    L_Y_DIV2, &i, &j) < 0){
		      goto after_gbsc;
		    }
		  }
