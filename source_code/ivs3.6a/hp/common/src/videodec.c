/**************************************************************************
*          Copyright 1995 by Jian Zhang & Ian West                         *
...
*                                   E-mail: west@jrc.flinders.edu.au       *
...
*--------------------------------------------------------------------------*
* Dick Zetterberg       | 94/11/11  | XvPutVideo is now only done once     *
* dick@ee.uts.edu.au    |           | and the whole image is grabbed rather*
* (d89-dze@nada.kth.se) |           | than just one corner like before.    *
****************************************************************************
...
#define PAL_WIDTH               636
#define PAL_HEIGHT              520     
  ...
  
{
    ...
    if (firstTime)
      {
	XvPutVideo(dpy, port, win, gc, 0, 50,
		   PAL_WIDTH, PAL_HEIGHT, 0, 0, winWidth, winHeight);
	firstTime = FALSE;
      }
    ...
    u_char         Cb_data[][COL_MAX_DIV2];   
{
	...
	y = ((  FIX(0.29900)*r  + FIX(0.58700)*g + FIX(0.11400)*b + ONE_HALF)
	     >> SCALEBITS);
     ...
     S_PARAM *param;
{
  ...
  BOOLEAN LOCAL_DISPLAY=TRUE; /* Always true */
  ...
  dpy = XOpenDisplay(param->display);
  if(!dpy){
    fprintf(stderr,"Cannot open display %s\n", param->display);
    exit(-1);
  }
  
  windowOpened = WindowSetup(dpy, &width, &height, &mainWin, &gc, &xvport);
  ...
  do{
    if(param->FREEZE == FALSE) 
      if(param->COLOR)
	(*get_image)(dpy, mainWin, xvport, gc, width, height, new_Y, new_Cb,
		     new_Cr); 
      else
    ...
    CONTINUE = HandleCommands(param, &TUNING);
