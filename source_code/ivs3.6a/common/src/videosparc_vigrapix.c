...
/**************************************************************************\
*                                                                          *
*  File              : videosparc_vigrapix.c                               *
*  Last modification : 1995/3/9                                            *
*  Author            : Thierry Turletti                                    *
*--------------------------------------------------------------------------*
*  Description :  Video interface for the VIGRAPIX framegrabber.           *
*                                                                          *
*--------------------------------------------------------------------------*
...
#ifdef VIGRAPIX
#ident "@(#) videosparc_vigrapix.c 1.1@(#)"
...
void sun_vigrapix_video_encode(param)
     S_PARAM *param;

{
  ...
  if((new_Y  = (u_char *) malloc(LIG_MAX*COL_MAX)) == NULL){
    perror("vigrapix_encode'malloc");
    exit(1);
  }
  if((new_Cb  = (u_char *) malloc(LIG_MAX_DIV2*COL_MAX_DIV2))
     == NULL){
    perror("vigrapix_encode'malloc");
    exit(1);
  }
  if((new_Cr  = (u_char *) malloc(LIG_MAX_DIV2*COL_MAX_DIV2))
     == NULL){
    perror("vigrapix_encode'malloc");
    exit(1);
  }

  VigraPixInitialize(&(param->vp), param->video_port, param->video_size,  
		     param->secam_format, param->COLOR, &NG_MAX, &L_lig,
...
/*  fread((char *) new_Y, 1, 182, F_input); */
  ...
  do{
...
#ifdef YUV_4_1_1
      {
	if(fread((u_char *)new_Y, 1, yuv411_size, F_input) <= 0)
	  goto stop;
	if(fread((u_char *)new_Cb, 1, yuv411_size_div4, F_input) <= 0)
	  goto stop;
	if(fread((u_char *)new_Cr, 1, yuv411_size_div4, F_input) <= 0)
	  goto stop;
      }
#else
      if(param->video_size == CIF_SIZE){
	if(fread((u_char *)new_Y, 1, L_lig*L_col, F_input) <= 0)
	  goto stop;
	usleep(200000);
      }else{
	register int l;
	for(l=0; l<L_lig; l++)
	  if(fread(((u_char *)new_Y+l*cif_size), 1, L_col, F_input) <= 0)
	    goto stop;
	usleep(100000);
      }
#endif /* YUV_4_1_1 */	
      ...
      if(!param->FREEZE){
	if(param->COLOR)
	  (*get_image)(param->vp, (u_char *)new_Y, (u_char *)new_Cb, 
		       (u_char *)new_Cr);
	else
	  (*get_image)(param->vp, (u_char *)new_Y);
      }
#endif /* INPUT_FILE */
#ifdef OUTPUT_FILE
#ifdef GRAB_COLOR
      if(param->video_size == CIF_SIZE){
	if(fwrite((char *) (u_char *)new_Y, 1, dim_imageY, F_original_file) !=
	   dim_imageY){
	  perror("fwrite output file");
	}
	if(fwrite((char *) (u_char *)new_Cb, 1, dim_imageC, F_original_file) !=
	   dim_imageC){
	  perror("fwrite output file");
	}
	if(fwrite((char *) (u_char *)new_Cr, 1, dim_imageC, F_original_file) !=
	   dim_imageC){
	  perror("fwrite output file");
	}
      }else{
	...
	for(l=0; l<L_lig; l++){
	  if(fwrite((char *) ((u_char *)new_Y+l*cif_size), 1, L_col, 
		    F_original_file) != L_col)
	    perror("fwrite output file");
	}
	for(l=0; l<LigDiv2; l++){
	  if(fwrite((char *) ((u_char *)new_Cb+l*qcif_size), 1, ColDiv2,
		    F_original_file) != ColDiv2)
	    perror("fwrite output file");
	}
	for(l=0; l<LigDiv2; l++){
	  if(fwrite((char *) ((u_char *)new_Cr+l*qcif_size), 1, ColDiv2,
		    F_original_file) != ColDiv2)
	    perror("fwrite output file");
	}
      }
	...
	for(l=0; l<L_lig; l++)
      ...
      encode_h261(param, (u_char *)new_Y, (u_char *)new_Cb, (u_char *)new_Cr, 
		  image_coeff[0], NG_MAX);

      if(param->LOCAL_DISPLAY && !ICONIC_DISPLAY){
	
	build_image((u_char *)new_Y, (u_char *)new_Cb, (u_char *)new_Cr, 
	...
	SetVigraPixPort(param->vp, param->video_port);
      }
      if(video_size != param->video_size){
	register int w , h ;

	video_size = param->video_size;
	VigraPixInitialize(&(param->vp), param->video_port, video_size, 
			   param->secam_format, param->COLOR, &NG_MAX, 
			   &L_lig, &L_col, &Lcoldiv2,
...
*/
	...
	case MapNotify :
	  ...
	  if (evt.xmap.window == icon) {
	    ICONIC_DISPLAY = TRUE ;
	    build_icon_image(icon, param->video_size, (u_char *)new_Y, 
			     (u_char *)new_Cb, (u_char *)new_Cr,
