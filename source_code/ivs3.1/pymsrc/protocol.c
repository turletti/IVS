...
int SendCSDESC(s, addr, len_addr, video_encoding, audio_encoding, 
	       feedback_port, sin_addr, name)
     int s;
     struct sockaddr_in *addr;
     int len_addr;
     ...
     char *name;
{
  ...
  * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  * |0|  CDESC=32   |   length=4    |0|0|  content  |      MBZ      |
  * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  ...
  * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  * |1|  SDESC=33   |   length=?    |     id unique within host     |
  * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  ...
  */
  ...
  if(FIRST){
    ...
    buf[3] = 0x00;
    buf[6] = 0x00; /* clock quality ?? */
    buf[7] = 0x00;
    bcopy("H261", (char *)&buf[8], 4);
    buf[12] = RTP_IVS_VERSION;
    buf[14] = 0x00;
    buf[15] = 0x00;
    buf[16] = 0xa1;
    length_sdesc = 3 + (strlen(name) / 4);
    buf[17] = length_sdesc;
    buf[18] = 0x00;
    buf[19] = 0x00;
    bcopy((char *)&sin_addr, (char *)&buf[20], 4);
    strcpy((char *)&buf[24], name);
    len = (4 + length_sdesc) << 2;
    FIRST=FALSE;
  }
  buf[4] = (u_char)(feedback_port >> 8);
  buf[5] = (u_char)(feedback_port & 0xff);
  buf[13] = audio_encoding;
  buf[14] = video_encoding;
