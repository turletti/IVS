...
*  Release : 1.0.0						           *
*--------------------------------------------------------------------------*
*  Description : CODEC audio + video + qauntizer & ffct choices.           *
*                                                                          *
\**************************************************************************/
...
char *name;
  {
    fprintf(stderr, 
      "Usage : %s -q 3to15 -ffct 4or8 [-o] [hostname]\n", name);
    exit(1);
  }
...
static void CallbackDVideo(w, null, reason)
    ...
    XmPushButtonCallbackStruct *reason;
  {
    ...
    if(t[i].video_decoding == FALSE){
      if(pipe(dv_fd[i]) < 0){
	fprintf(stderr, "CallbackDVideo: Cannot create pipe\n");
	return;
      }
      if((idv=fork()) == -1){
	fprintf(stderr,"CallbackDVideo: Cannot fork decoding video process\n");
	return;
      }
      if(!idv){
	close(dv_fd[i][1]);
    ...
    XmPushButtonCallbackStruct *reason;
{
  ...
  if(audio_set==FALSE){
    audio_set=TRUE;
    if(pipe(da_fd) < 0){
      fprintf(stderr, "Audio Decoding : Cannot create pipe\n");
      return;
    }
    if((id=fork()) == -1){
      fprintf(stderr,"Audio Decoding : Cannot fork !\n");
      return;
    }
    if(id){
      close(da_fd[1]);
...
static void CallbackBri(w, null, reason)
    ...
    XmScaleCallbackStruct *reason;
  {
    ...
    if((lwrite=write(scaledv_fd[1], (char *)luth, 256)) < 0){
      printf(stderr, "CallbackBri : error when writing on pipe\n");
      exit(1);
    }
  }
...
static void CallbackCon(w, null, reason)
    ...
    XmScaleCallbackStruct *reason;
  {
    ...
    if((lwrite=write(scaledv_fd[1], (char *)luth, 256)) < 0){
      printf(stderr, "CallbackCon: error when writing on pipe\n");
      exit(1);
    }
  }
...
static void CallbackDefault(w, argv, reason)
    ...
    XmPushButtonCallbackStruct *reason;
  {
    ...
    if((lwrite=write(scaledv_fd[1], (char *)luth, 256)) < 0){
      printf(stderr, "CallbackDefault: error when writing on pipe\n");
      exit(1);
    }
  }
...
static void CallbackVideo(w, argv, reason)
    ...
    XmPushButtonCallbackStruct *reason;
  {
    ...
    if(!START_EVIDEO){
      if(pipe(ev_fd) < 0){
	fprintf(stderr, "CallbackVideo : Cannot create pipe\n");
	return;
      }
	...
	fprintf(stderr,"No input video available on any port\n");
	...
	return;
      ...
      if((id=fork()) == -1){
	fprintf(stderr,"CallbackVideo : Cannot fork !\n");
	MappWidget(ButtonVideo);
	return;
      }
	...
	video_encode(IP_GROUP, VIDEO_PORT, ttl, col, lig, quality, ev_fd, 
		     CUTTING, video_port, quantizer, nffct);
      }
    }else{
      SendPP_QUIT(ev_fd);
      START_EVIDEO=FALSE;
...
#else
    fprintf(stderr, "Sorry, VIDEOPIX hardware is not installed\n");
#endif /* VIDEOPIX */
  }
...
static void CallbackAudio(w, argv, reason)
    ...
    XmPushButtonCallbackStruct *reason;
  {
    ...
    if(!START_EAUDIO){
      if(pipe(ea_fd) < 0){
	fprintf(stderr, "CallbackAudio : Cannot create pipe\n");
	return;
      }
      if((id=fork()) == -1){
	fprintf(stderr,"CallbackAudio : Cannot fork !\n");
	return;
      }
      /* fathers's process goes on , son's process starts encoding */
      if(id){
     ...
     char **argv;
{
  ...
  XtAppContext app;
  Display  *disp;
  Widget  appShell, main_win, dwa, menubar, input_menu, output_menu,
  video_menu, input_entry, output_entry, ButtonQuit, 
  ...
  narg = 1;
  while(narg != arg){
    if(strcmp(argv[narg], "-q") == 0){
      quantizer = atoi(argv[++narg]);
      narg ++;
      continue;
    }else{
      if(strcmp(argv[narg], "-ffct") == 0){
	nffct = atoi(argv[++narg]);
	narg ++;
	continue;
      }else{
  ...
  if(UNICAST){
	...
	exit(-1);
      ...
      bcopy((char *)hp->h_addr, (char *)&tadd.sin_addr, 4);
      strcpy(IP_GROUP, inet_ntoa(tadd.sin_addr));
    }else
  ...
  video_cutting = XtCreateManagedWidget ("One packet <-> One GOB", 
				xmPushButtonWidgetClass, video_menu, args, n);
  video_nocutting = XtCreateManagedWidget ("One packet <-> One Image", 
				xmPushButtonWidgetClass, video_menu, args, n);
