...
ReportToCoder(num)
    int num;
  {
	...
	if((add_long=inet_addr(host_coder)) == -1){
	  ...
	  addr_coder.sin_addr.s_addr = add_long;
	}
      }else
...

{
	...
	* BLOCK LAYER *
	 ...
	 *  Y1, Y2, Y3, and Y4. Cb and Cr are not encoded.
	 */
         if(P & 0x20){
	   decode_bloc_8x8(chaine_codee, block, state_tcoeff, &i, &j, &cm,CBC);
	 ...
	 */
	 if(DOUBLE){
	   ...
	   register int l, c;
	   register u_char aux, *tab_Y_x, *im_pix1, *im_pix2, temp;

	   if(depth == 4){
	     offset1 = L_Y - 16;
	     offset2 = L_col - 16;
	     tab_Y_x = &(tab_Y[y_mb][x_mb]);
	     im_pix1 = &(im_pix[(y_mb*L_col)+x_mb]);
	     im_pix2 = &(im_pix[((y_mb+1)*L_col)+x_mb]);
	     for(l=0; l<16; l++){
	       for(c=0; c<16; c++){
		 temp = (*tab_Y_x / 16);
		 aux=map_lut[(temp << 4) + temp];
		 tab_Y_x ++;
		 *im_pix1++ = aux;
		 *im_pix2++ = aux;
	       }
	   ...
	   if(depth == 4){
	     ...
	     offset2 = (L_col - 16)/2;
	     tab_Y_x = &(tab_Y[y_mb][x_mb]);
	     im_pix_x = &(im_pix[((y_mb*L_col)+x_mb)/2]);
	     for(l=0; l<16; l++){
	       for(c=0; c<8; c++){
		 *im_pix_x++ = map_lut[((*tab_Y_x/16)<<4) + 
				       (*(tab_Y_x+1)/16)];
		 tab_Y_x += 2;
	       }
	       im_pix_x += offset2;
